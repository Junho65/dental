...
This project provides a local-first dental X-ray training and inference workflow:
...
The evaluator-facing script index and recommended execution paths are documented in
...
For a CUDA-specific PyTorch wheel, install the wheel appropriate for the evaluation machine before
...
```bash
python scripts/data/merge_yolo_detection_datasets.py --base data/detection_merged --extra data/detection_umfih --out data/detection_merged_umfih
```
...
By default, `python scripts/training/train_detection.py` trains on an already prepared hierarchical dataset:
...
### `scripts/data/prepare_detection_dataset.py`
...
### `scripts/data/prepare_cariesxrays_yolo.py`
...
```bash
python scripts/data/prepare_cariesxrays_yolo.py --raw data/raw/cariesxrays --out data/detection_cariesxrays --stem-prefix cx_
```

### `scripts/data/prepare_umfih_yolo.py`
...
```bash
python scripts/data/merge_yolo_detection_datasets.py --base data/detection --extra data/detection_cariesxrays --out data/detection_merged
python scripts/data/merge_yolo_detection_datasets.py --base data/detection_merged --extra data/detection_umfih --out data/detection_merged_umfih
python scripts/data/merge_yolo_detection_datasets.py --base data/detection_main_4class_no_cyst_no_periodontal --extra data/detection_kaggle_pediatric_selected_4class --out data/detection_main_4class_with_pediatric --yaml-name main_4class_with_pediatric.yaml
```
...
### `scripts/training/run_pediatric_service_finetune.py`

Purpose:
- Fine-tune the currently served 4-class detector starting from `artifacts/detection/serve/dental_4class_detection_best.pt`
...
- Promote the new `best.pt` into `artifacts/detection/serve/dental_4class_detection_best.pt` only when the gating rules pass
...
```bash
python scripts/training/run_pediatric_service_finetune.py
```

Useful dry-run style command when you want dataset prep and evaluation wiring without replacing the service weights:

```bash
python scripts/training/run_pediatric_service_finetune.py --skip-train --candidate-weights artifacts/detection/serve/dental_4class_detection_best.pt --no-promote
```

### Recommended execution order

1. `python scripts/data/prepare_detection_dataset.py`
2. `python scripts/data/prepare_cariesxrays_yolo.py --raw data/raw/cariesxrays --out data/detection_cariesxrays --stem-prefix cx_`
3. `python scripts/data/prepare_umfih_yolo.py`
4. `python scripts/data/merge_yolo_detection_datasets.py --base data/detection --extra data/detection_cariesxrays --out data/detection_merged`
5. `python scripts/data/merge_yolo_detection_datasets.py --base data/detection_merged --extra data/detection_umfih --out data/detection_merged_umfih`
6. `python scripts/data/prepare_hierarchical_detection_dataset.py --data data/detection_merged_umfih/merged_detection.yaml`
7. `python scripts/training/train_detection.py`
...
The training command defaults to:
...
```bash
python scripts/training/train_detection.py --name yolov8s_experiment
python scripts/training/train_detection.py --batch 4
```

Recommended model comparison on GTX 1660 6GB:

```bash
python scripts/training/train_detection.py --model yolov8n.pt --name yolov8n_hierarchical_baseline --batch 16 --imgsz 416
python scripts/training/train_detection.py --model yolov8s.pt --name yolov8s_hierarchical --batch 8 --imgsz 416
python scripts/training/train_detection.py --model yolo11s.pt --name yolo11s_hierarchical --batch 8 --imgsz 416
```

Run a 1-epoch smoke test before full training when trying a new model:

```bash
python scripts/training/train_detection.py --model yolov8s.pt --name yolov8s_smoke --epochs 1 --batch 8 --imgsz 416
python scripts/training/train_detection.py --model yolo11s.pt --name yolo11s_smoke --epochs 1 --batch 8 --imgsz 416
```

The decision metric is `metrics/mAP50-95(B)` in each run's `results.csv`. The existing `yolov8n_hierarchical2` reference is about `0.126` at epoch 10 and about `0.173` final, so a larger model should beat both the early and final reference before replacing `artifacts/detection/serve/dental_4class_detection_best.pt`.
...
```bash
python scripts/training/train_detection.py --data data/detection_hierarchical/hierarchical_detection.yaml --oversample-class impacted_tooth --deep-caries-balance
python scripts/training/train_detection.py --data data/detection_merged_umfih/merged_detection.yaml --deep-caries-balance
```
...
```bash
python scripts/data/prepare_hierarchical_detection_dataset.py --data data/detection_merged/merged_detection.yaml
python scripts/training/train_detection.py --data data/detection_hierarchical/hierarchical_detection.yaml --no-deep-caries-balance
```
...
```bash
python scripts/data/prepare_followup_crop_dataset.py --data data/your_followup_dataset/data.yaml --class-names class_a class_b --out data/followup_custom
```

Then train with the same class order:

```bash
python scripts/training/train_severity_classifier.py --train-csv data/followup_custom/train.csv --val-csv data/followup_custom/val.csv --output-dir artifacts/severity/periapical_followup --class-names class_a class_b
```
...
```bash
python scripts/evaluation/eval_severity_classifier.py --checkpoint artifacts/severity/serve/bone_loss/best.pt --test-csv data/severity_periodontal/bone_loss/test.csv --out-json artifacts/severity/serve/bone_loss/test_metrics.json
python scripts/evaluation/eval_severity_classifier.py --checkpoint artifacts/severity/serve/furcation_involvement/best.pt --test-csv data/severity_periodontal/furcation_involvement/test.csv --out-json artifacts/severity/serve/furcation_involvement/test_metrics.json
```

To retrain and reevaluate both periodontal severity classifiers in one run:

```bash
python scripts/training/run_periodontal_severity_retrain.py --epochs 20 --selection-metric val_f1_macro --early-stopping-metric val_f1_macro
```
...
```bash
python scripts/training/train_detection.py --name yolov8s_live
...
```bash
python scripts/evaluation/eval_detection.py --weights artifacts/detection/yolov8s_hierarchical/weights/best.pt --data data/detection_hierarchical/hierarchical_detection.yaml --imgsz 416
```
